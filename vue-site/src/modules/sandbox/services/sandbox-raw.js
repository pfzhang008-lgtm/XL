import * as THREE from 'three';
    ...
    function normalizeLibraryVersion(rawVersion) {
      const value = String(rawVersion || "").trim().toLowerCase();
      if (["child", "children", "kid", "kids", "kid-version", "children-version", "孩子", "儿童"].includes(value)) {
        return "child";
      }
      if (["adult", "adults", "grown", "adult-version", "成人"].includes(value)) {
        return "adult";
      }
      return "adult";
    }
    ...
    const requestedLibraryVersion =
      urlParams.get("people")
      || urlParams.get("personSet")
      || urlParams.get("personVersion")
      || urlParams.get("library")
      || urlParams.get("version")
      || "adult";
    const activeLibraryVersion = normalizeLibraryVersion(requestedLibraryVersion);
    const paletteData = activeLibraryVersion === "child" ? buildChildPaletteData() : adultPaletteData;
    ...
    const btnTeaching = document.getElementById("btnTeaching");
    if (btnTeaching) {
      btnTeaching.addEventListener("click", () => {
        window.location.href = "?people=child";
      });
    }
    ...
    if (btnNormal) {
      btnNormal.addEventListener("click", () => {
        window.location.href = "?people=adult";
      });
    }

    // 3. 灯光
    const ambientLight = new THREE.AmbientLight(0xffffff, 1.6); // 增加环境光以调淡阴影
    scene.add(ambientLight);

    const dirLight = new THREE.DirectionalLight(0xfff5e6, 1.2); // 降低主光源强度，缩小明暗对比，使阴影变浅
    ...
    function createPieceMesh(meta, options = {}) {
      ...
      const mat = new THREE.MeshStandardMaterial({ 
        color: meta.hexColor, 
        roughness: 0.65,     // 稍微降低粗糙度，让光影更明显，增强立体感
        metalness: 0.05,     // 增加一点点金属感辅助立体感
        map: texture 
      });
    ...
    const thumbLight = new THREE.DirectionalLight(0xffffff, 1.45);
    ...
    function resizePalette() {
      ...
      // 使用 app 容器的尺寸，以正确处理移动端的 CSS 旋转 (90deg)
      // 在移动端旋转后，app.clientHeight 是物理宽度，app.clientWidth 是物理高度
      const appNode = document.querySelector('.app');
      const appW = appNode.clientWidth;
      const appH = appNode.clientHeight;
      ...
      // 3. 限制人物栏总宽度不超过屏幕可用宽度的 90% (留出空间给标签)
      const maxSidebarWidth = appW * 0.92;
      ...
      sidebar.style.width = `${requiredSidebarWidth}px`;
    ...
    window.addEventListener('resize', () => {
      ...
      setTimeout(updateNavPosition, 50);
      setTimeout(updateNavPosition, 400); // 动画结束后再次校准
    });
